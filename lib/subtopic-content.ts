import type { SubtopicKnowledge } from "./learning-types";
...
export function buildLessonFromSubtopic(subtopic: SubtopicKnowledge) {
  ...
  const academicDefinition =
    termSentences.length > 0 ? termSentences.join(" ") : definitionSentence;
  ...
  if (termEntries.length > 0) {
    const deepTerms = termEntries.slice(0, 3).map(([term, definition]) => `${term} - ${definition}`);
  ...
  if (deepExamples.length > 0) {
    deepEssayParts.push(`<p><strong>Real-life connections:</strong></p>${listHtml(deepExamples)}`);
  }
  if (misconceptions[0]) {
    deepEssayParts.push(`<p><strong>Common confusion:</strong> ${ensureSentence(misconceptions[0])}</p>`);
  }
  const advancedDetail = deepDives[0] || basePoints[2] || "";
  if (advancedDetail) {
    deepEssayParts.push(`<p><strong>Advanced detail:</strong> ${ensureSentence(advancedDetail)}</p>`);
  }
  if (objectives[0]) {
    deepEssayParts.push(
